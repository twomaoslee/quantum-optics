...
学生入口为 `dist/index.html`，当前发布第一讲在线讲义、课堂课件、第一讲课件 PDF、16讲目录、补充阅读与来源。第二讲资料在教师授课后再发布，本地备课文件保留。

`publication.json` 控制允许发布的讲次，不能根据源文件存在与否自动发布。第二讲当前不进入发布目录、导航链接或搜索结果；教师明确要求发布后才将其加入 `published_lectures` 并从 `after_class_lectures` 移出。
...
- 补齐第一讲视频海报缺失的输出副本。
- 每个讲义页面提供返回课程首页的入口。
- PDF 是现有第一讲课件导出，49页；不是长文讲义 PDF，不能用它替代之后的讲义打印版。
- 长文 PDF 与离线包尚未制作，本轮不虚设下载链接。
