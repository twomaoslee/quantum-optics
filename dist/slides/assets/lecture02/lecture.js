...
// A classroom simulation runs only while its slide is visible.
(() => {
 const connect=()=>{
  if(!window.Reveal?.isReady())return setTimeout(connect,100);
  Reveal.on('slidechanged',()=>{
   document.querySelectorAll('.lab-frame').forEach(frame=>frame.contentWindow?.postMessage({type:'lecture-visibility',visible:frame.closest('section')===Reveal.getCurrentSlide()},'*'));
  });
 };
 connect();
})();
