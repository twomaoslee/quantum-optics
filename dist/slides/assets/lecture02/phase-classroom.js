...
function fit(){
  const scale=Math.min(window.innerWidth/1216,window.innerHeight/542);
  stage.style.transform='scale('+scale+')';
  stage.style.left=(window.innerWidth-1216*scale)/2+'px';
  stage.style.top=(window.innerHeight-542*scale)/2+'px';
}
...
function phasor(svg,sign,phi){
  ...
  const S=170,xx=v=>100+S*v,yy=v=>100-S*v;
  ...
  svg.innerHTML='<defs>'+colors.map((c,i)=>'<marker id="'+id+i+'" markerUnits="userSpaceOnUse" markerWidth="12" markerHeight="12" refX="10" refY="6" orient="auto"><path d="M1 1 L10 6 L1 11" fill="none" stroke="'+c+'" stroke-width="2"/></marker>').join('')+'</defs>'+
    '<circle cx="'+xx(.5)+'" cy="100" r="85" fill="none" stroke="#d6e3ed" stroke-dasharray="4 5"/>'+
    '<path d="M50 100H365 M100 198V2" stroke="#b7c8d5" stroke-width="1.2"/>'+
    '<text x="372" y="107">Re</text><text x="108" y="22">Im</text><text x="78" y="124">0</text><text x="175" y="124">½</text><text x="266" y="124">1</text>'+
